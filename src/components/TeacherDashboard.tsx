...
import { collection, onSnapshot } from "firebase/firestore"; // Changed from getDocs
...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      studentData.sort((a, b) => {
        if (
          a.mentalHealthStatus === "negative" &&
          b.mentalHealthStatus !== "negative"
        )
          return -1;
        if (
          a.mentalHealthStatus !== "negative" &&
  ...
  const positiveStudents = students.filter(
    (s) => s.mentalHealthStatus === "positive"
  ).length;
  const negativeStudents = students.filter(
    (s) => s.mentalHealthStatus === "negative"
  ).length;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {totalStudents > 0
                  ? Math.round((positiveStudents / totalStudents) * 100)
                  : 0}
                % of total
              </p>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {totalStudents > 0
                  ? Math.round((negativeStudents / totalStudents) * 100)
                  : 0}
                % of total
              </p>
            </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Filter and search through student records
            </CardDescription>
          </CardHeader>
          <CardContent>
                        ...
                        <Dialog>
                          ...
                          {selectedStudent && (
                            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                              <DialogHeader>
                                <DialogTitle>
                                  Student Reports - {selectedStudent.name}
                                </DialogTitle>
                                <DialogDescription>
                                  Class {selectedStudent.class}
                                  {selectedStudent.division} •{" "}
                                  {selectedStudent.reportsCount || 0} total
                                  reports
                                </DialogDescription>
                              </DialogHeader>
                              ...
                              <Tabs defaultValue="analysis" className="w-full">
                                    ...
                                    <CardContent className="space-y-4">
                                      <div className="grid grid-cols-3 gap-4">
                                        <div>
                                          ...
                                          <div className="mt-1">
                                            {getScoreBadge(
                                              selectedStudent.depressionScore,
                                              "depression"
                                            )}
                                          </div>
                                        </div>
                                ...
                                <TabsContent
                                  ...
                                  className="space-y-4"
                                >
                                  {(selectedStudent as any).essays?.map(
                                    (essay: any) => (
                                      <Card key={essay.id}>
                                        <CardHeader>
                                          <div className="flex items-center justify-between">
