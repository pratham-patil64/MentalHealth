...
import {
    ...
    User as UserIcon, LogOut, Calendar, X, Wind, Flower, Star, Eye, Hand, Ear, Sun, Fingerprint, ArrowLeft, ClipboardList, BookOpen
} from "lucide-react";
...
const StudentDashboard = ({ user, googleAccessToken }: StudentDashboardProps) => {
    ...
    const [view, setView] = useState<"dashboard" | "profile" | "story">("dashboard");
    ...
    const fetchStudentData = useCallback(async () => {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                if (view !== 'story' && (!currentStudentData.school || !currentStudentData.parentPhone || !currentStudentData.class)) {
                    setView("profile");
                }
            } else {
                setView("profile");
    ...
    const calculateAndSaveUnifiedScores = useCallback(async () => {
        ...
        // Fetch the latest chat data for the month
        const now = new Date();
        ...
        let totalAnxietyScore = 0, totalDepressionScore = 0, totalStressScore = 0;
        monthlyChatsSnapshot.docs.forEach(d => {
            const data = d.data().scores;
            if (data) {
                totalAnxietyScore += data.anxiety || 0;
                totalDepressionScore += data.depression || 0;
                totalStressScore += data.stress || 0;
            }
        });

        const chatCount = monthlyChatsSnapshot.size > 0 ? monthlyChatsSnapshot.size : 1;
        const avgChatAnxiety = totalAnxietyScore / chatCount;
        const avgChatDepression = totalDepressionScore / chatCount;
        const avgChatStress = totalStressScore / chatCount;

        // Behavioral scores (0-100, 100 is GOOD) - Default to 50 if unavailable
        const activityScore = behavioralScores?.activityScore ?? 50;
        const sleepScore = behavioralScores?.sleepScore ?? 50;

        // Convert behavioral scores to "negative factors" (0-100, 100 is BAD)
        const activityFactor = 100 - activityScore;
        const sleepFactor = 100 - sleepScore;

        // Combine scores: 60% self-report (chat), 40% behavioral (Fit)
        // DASS-21 Max scores: Stress=42, Anxiety=21, Depression=42
        const finalStressScore = Math.round((avgChatStress / 42 * 100) * 0.6 + (sleepFactor * 0.2 + activityFactor * 0.2));
        const finalAnxietyScore = Math.round((avgChatAnxiety / 21 * 100) * 0.6 + (sleepFactor * 0.4));
        const finalDepressionScore = Math.round((avgChatDepression / 42 * 100) * 0.6 + (activityFactor * 0.3 + sleepFactor * 0.1));

        const finalScores = {
            stress: Math.min(100, Math.max(0, finalStressScore)),
            anxiety: Math.min(100, Math.max(0, finalAnxietyScore)),
            depression: Math.min(100, Math.max(0, finalDepressionScore))
        }

        // Save to Firestore under a nested 'scores' object
        const studentDocRef = doc(db, "students", user.uid);
        await setDoc(studentDocRef, { scores: finalScores }, { merge: true });

        // Update local state for immediate UI feedback
        setStudentData(prev => prev ? {
            ...prev,
            stressScore: finalScores.stress,
            anxietyScore: finalScores.anxiety,
            depressionScore: finalScores.depression,
        } : null);
    }, [user, behavioralScores, studentData]);

    // This effect triggers the calculation when Google Fit data arrives
    useEffect(() => {
        if (behavioralScores) {
    ...
    // which can cause infinite loops when passed as a prop to child components.
    ...
    const handleCheckinComplete = () => {
        ...
        // Refetch data to get the latest chat, then recalculate scores
    ...
    const handleStoryComplete = () => {
        fetchStudentData(); // Refresh data after story mode
        setView("dashboard");
    }

    // Timer for calming exercises
    useEffect(() => {
            ...
            <div className="min-h-screen bg-gradient-background">
                <header className="bg-card/80 backdrop-blur-md border-b border-border sticky top-0 z-40">
                    <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-foreground">{view === 'profile' ? "Your Profile" : (view === 'story' ? "Weekly Story" : `Welcome, ${studentData?.name || "Student"}!`)}</h1>
                                <p className="text-muted-foreground">{view === 'profile' ? "Please keep your details up to date." : (view === 'story' ? "Your choices shape your journey." : "How are you feeling today?")}</p>
                            </div>
                        </div>
                        <div className="flex items-center space-x-2">
                            {view !== 'dashboard' && <Button variant="ghost" size="icon" onClick={() => setView('dashboard')}><ArrowLeft className="w-5 h-5" /></Button>}
                            <Button variant="ghost" size="icon" onClick={() => setView(v => v === 'profile' ? 'dashboard' : 'profile')}><UserIcon className="w-5 h-5" /></Button>
                ...
                <main className="container mx-auto px-6 py-8 max-w-7xl">
                    {view === 'profile' ? (
                        ...
                        <StoryMode user={user} studentData={studentData} onStoryComplete={handleStoryComplete} />
                    ) : (
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                <div className="grid md:grid-cols-3 gap-6">
                                    <Card
                                        ...
                                        className={`border-0 shadow-card hover:shadow-glow transition-all duration-300 group ${hasChattedThisWeek
                                                ? 'bg-gradient-to-br from-gray-500 to-gray-600 cursor-not-allowed'
                                                : 'bg-gradient-wellness hover:scale-105 cursor-pointer'
                                            }`}
                                    >
                                        <CardHeader className="text-center p-4 flex flex-col items-center justify-center">
                                            <div className="mx-auto w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-2 transition-transform duration-300">
                                                {hasChattedThisWeek ? <Check className="w-7 h-7 text-white" /> : <ClipboardList className="w-7 h-7 text-wellness-foreground" />}
                                            </div>
                                            <CardTitle className="text-xl text-white">
                                                {hasChattedThisWeek ? "Check-in Done" : "Weekly Check-in"}
                                            </CardTitle>
                                        </CardHeader>
                                    </Card>

                                    <Card className="border-0 shadow-card hover:shadow-glow transition-all duration-300 hover:scale-105 bg-gradient-primary cursor-pointer group" onClick={() => setView('story')}>
                                        <CardHeader className="text-center p-4 flex flex-col items-center justify-center">
                                            <div className="mx-auto w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-2 transition-transform duration-300"><BookOpen className="w-7 h-7 text-primary-foreground" /></div>
                                            <CardTitle className="text-xl text-primary-foreground">Weekly Story</CardTitle>
                                        </CardHeader>
                                    </Card>
                                    ...
                                    <Card className="border-0 shadow-card hover:shadow-glow transition-all duration-300 hover:scale-105 bg-gradient-calm cursor-pointer group" onClick={handleOpenGeneralChat}>
                                        <CardHeader className="text-center p-4 flex flex-col items-center justify-center">
                                            <div className="mx-auto w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-2 transition-transform duration-300"><MessageCircle className="w-7 h-7 text-calm-foreground" /></div>
                                            <CardTitle className="text-xl text-calm-foreground">AI Companion</CardTitle>
                                        </CardHeader>
                                    </Card>
                                </div>
                                ...
                                <Card className="border-0 shadow-card bg-card/50 backdrop-blur-sm">
                                    <CardHeader>
                                        <CardTitle className="text-2xl text-foreground flex items-center"><Brain className="w-6 h-6 mr-3 text-primary" />Your Wellness Scores</CardTitle>
                                        <CardDescription>Scores are on a 0-100 scale, where lower is better. They are updated based on your weekly activities.</CardDescription>
                                    </CardHeader>
