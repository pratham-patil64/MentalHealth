...
import {
  ...
  User as UserIcon, LogOut, Calendar, TrendingUp, X, Wind, Flower, Star, Eye, Hand, Ear, Sun, Fingerprint
} from "lucide-react";
...
// Data for the 5-4-3-2-1 method steps
...
const StudentDashboard = ({ user }: StudentDashboardProps) => {
  ...
  // Countdown timer for breathing exercises
  useEffect(() => {
        ...
        <header className="bg-card/80 backdrop-blur-md border-b border-border sticky top-0 z-40">
           <div className="container mx-auto px-6 py-4 flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-foreground">Welcome, {studentData?.name || "Student"}!</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              ...
              <Card className="border-0 shadow-card bg-card/50 backdrop-blur-sm">
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Interact with the weekly journal application below.
                            </CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                    <iframe
                        src="https://1eecb2f21dd0d573d9.gradio.live"
                        className="w-full h-[600px] border-0 rounded-xl"
                        title="Gradio Journal App"
                    ></iframe>
                </CardContent>
              </Card>
            ...
            <aside className="space-y-6">
              <Card className="border-0 shadow-card bg-gradient-primary">
                <CardHeader className="text-center">
                  ...
                  <CardDescription className="text-primary-foreground/80"><TrendingUp className="w-4 h-4 inline mr-1" />+5 from last week</CardDescription>
                </CardHeader>
              </Card>
              <Card className="border-0 shadow-card bg-card/50 backdrop-blur-sm">
                <CardHeader>
                  <CardTitle className="text-xl text-foreground flex items-center"><History className="w-5 h-5 mr-2 text-primary" />Previous Essays</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  {previousEssays.map((essay) => (
                    <div key={essay.id} className="p-4 rounded-xl bg-muted hover:bg-muted/80 transition-colors cursor-pointer group">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h4 className="font-medium text-foreground group-hover:text-primary transition-colors">{essay.title}</h4>
                  ...
                  <Button variant="ghost" className="w-full text-primary hover:bg-primary/10">View All Essays</Button>
                </CardContent>
              </Card>
      ...
      {isChatOpen && (
       ...
       <style>{`
        @keyframes breathe-478 {
          ...
          100% { transform: scale(0.8); } /* Exhale 8s (19/19) */
        }
