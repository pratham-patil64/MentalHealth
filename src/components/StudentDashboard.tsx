...
import { signOut, User } from "firebase/auth"; // We need the 'User' type for props
...
import {
  ...
  User as UserIcon, LogOut, Calendar, TrendingUp, X
} from "lucide-react";

// This interface defines the structure of your student data in Firestore
interface Student {
  uid: string; name: string; email: string; class: string; division: string; gender: string;
}

// This defines the props the component will receive from App.tsx
...
const StudentDashboard = ({ user }: StudentDashboardProps) => {
  ...
  const [responses, setResponses] = useState({ mood: "", challenges: "", happiness: "", sleep: "", thoughts: "" });
  const [isChatOpen, setIsChatOpen] = useState(false);
  const navigate = useNavigate();

  // This simplified useEffect fetches data using the user prop
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        // We use the user.uid from the prop to find the correct document
        const docRef = doc(db, "students", user.uid);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          // If the document is found, we set the student's data in the state
          setStudentData(docSnap.data() as Student);
        } else {
          console.error("CRITICAL: A logged-in user does not have a document in Firestore!");
        }
      } catch (error) {
        console.error("ERROR fetching student data from Firestore:", error);
      } finally {
        // This 'finally' block ensures the loading screen disappears even if there's an error
        setLoading(false);
      }
    };

    fetchStudentData();
  }, [user]); // This hook re-runs if the user prop ever changes

  
  // The rest of your component's functions and JSX are below
  ...
  const questions = [
    { id: "mood", question: "How would you describe your overall mood this week?", placeholder: "Share how you've been feeling overall..." },
    { id: "challenges", question: "What was the most challenging part of your week?", placeholder: "Tell us about any difficulties you faced..." },
    { id: "happiness", question: "Did anything make you feel especially happy or relaxed?", placeholder: "Share moments that brought you joy..." },
    { id: "sleep", question: "How did your sleep and energy levels feel throughout the week?", placeholder: "Describe your rest and energy patterns..." },
    { id: "thoughts", question: "Is there anything that's been on your mind a lot lately?", placeholder: "Share what's been occupying your thoughts..." }
  ];

  const previousEssays = [
    { id: 1, title: "My Summer Vacation", date: "2025-08-15", status: "Analyzed", mood: "positive" },
    { id: 2, title: "Climate Change Essay", date: "2025-08-10", status: "Analyzed", mood: "concerned" },
    { id: 3, title: "Future Goals", date: "2025-08-05", status: "Processing", mood: "hopeful" }
  ];
  ...
  const handleSubmitCheckin = async () => {
    if (!studentData) {
      alert("Could not identify student. Please try again.");
      return;
    }
    ...
    try {
      ...
      setResponses({ mood: "", challenges: "", happiness: "", sleep: "", thoughts: "" });
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-background">
          ...
          <div className="container mx-auto px-6 py-4 flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-foreground">
                  {/* This line will now correctly display the student's name */}
            ...
            <div className="flex items-center space-x-4">
              ...
              <Button
                ...
                onClick={async () => {
                  await signOut(auth);
                  // The App.tsx will automatically handle redirecting to the login page after sign out
                }}
              >
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card
                  className="border-0 shadow-card hover:shadow-glow transition-all duration-300 hover:scale-105 bg-gradient-calm cursor-pointer group"
            ...
            <aside className="space-y-6">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center mt-2">
                            <div className={`w-2 h-2 rounded-full mr-2 ${essay.mood === 'positive' ? 'bg-green-500' : essay.mood === 'concerned' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
              ...
              <Card className="border-0 shadow-card bg-gradient-calm">
                <CardHeader className="text-center">
                  ...
                  <Button
                    variant="secondary"
                    className="mt-4 bg-white/20 text-calm-foreground hover:bg-white/30"
                    onClick={handleOpenChat}
                  >
                    Start Conversation
                  </Button>
