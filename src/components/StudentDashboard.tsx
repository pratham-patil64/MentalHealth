    ...
    import {
        ...
        User as UserIcon, LogOut, Calendar, X, Wind, Flower, Star, Eye, Hand, Ear, Sun, Fingerprint, ArrowLeft, ClipboardList, PlayCircle
    } from "lucide-react";
    ...
    const StudentDashboard = ({ user, googleAccessToken }: StudentDashboardProps) => {
        ...
        const [isGameOpen, setIsGameOpen] = useState(false); // State for the MindCareGame
        ...
        const fetchStudentData = useCallback(async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    currentStudentData = {
                        ...data,
                        depressionScore: scores.depression ?? data.depressionScore ?? undefined,
                        anxietyScore: scores.anxiety ?? data.anxietyScore ?? undefined,
                        stressScore: scores.stress ?? data.stressScore ?? undefined,
                    } as Student;
        ...
        // --- Unified Scoring Function (Advanced Logic) ---
                    ...
                    <header className="bg-card/80 backdrop-blur-md border-b border-border sticky top-0 z-40">
                        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
                            ...
                            <div className="flex items-center space-x-2">
                                <Button variant="ghost" size="icon" onClick={() => setView(v => v === 'profile' ? 'dashboard' : 'profile')}>
    ...
    <CardContent><GoogleFit accessToken={googleAccessToken} onScoresCalculated={handleScoresCalculated} user={user} /></CardContent>
                                ...
                                <aside className="space-y-6">
                                    ...
                                    {/* Re-integrated Game Card */}
                                    <Card className="border-0 shadow-card bg-gradient-to-br from-purple-500 to-pink-500">
                                        <CardHeader className="text-center">
                                            <PlayCircle className="w-12 h-12 text-white mx-auto mb-4 animate-pulse" />
                                            <CardTitle className="text-xl text-white">Mindful Journey</CardTitle>
                                            <CardDescription className="text-white/80">Play a game to learn about making healthy choices.</CardDescription>
                                            <Button variant="secondary" className="mt-4 bg-white/20 text-white hover:bg-white/30" onClick={() => setIsGameOpen(true)}>
                                                Play Now
                                            </Button>
                                        </CardHeader>
                                    </Card>
                ...
                <Dialog open={isGameOpen} onOpenChange={setIsGameOpen}>
                    <DialogContent className="max-w-sm p-0 border-0 bg-transparent shadow-none">
                        <MindCareGame onClose={() => setIsGameOpen(false)} />
                    </DialogContent>
                </Dialog>
